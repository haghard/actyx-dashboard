...
trait KafkaSupport extends CassandraSupport { mixin: Actor with ActorLogging =>
        ...
        .mapAsync(4) { message => //parallelism should be configurable
          for {
            ...
            Option(result.one).fold(
                Json.obj(
                    "deviceId" -> queryParams._1,
                    "ma" -> "none",
                    "when" -> (formatter format queryParams._3),
                    "actlimit" -> s"${queryParams._4}:${queryParams._5}")) {
              row =>
                Json.obj("deviceId" -> queryParams._1,
                         "ma" -> row.getDouble("value").scale(3),
                         "when" -> (formatter format queryParams._3),
                         "actlimit" -> s"${queryParams._4}:${queryParams._5}")
            }
          }
